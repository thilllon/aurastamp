import { Box, Modal, TextField, IconButton, Button, Input } from '@mui/material';
...
export const ModalEncoder = ({
    ...
    const [hiddenImagePreviewUrl, setHiddenImagePreviewUrl] = useState<string | ArrayBuffer | null>('');
    ...
    const onSelectFile: ChangeEventHandler<HTMLInputElement> = async (ev) => {
        if (ev.target.files && ev.target.files.length > 0) {
            ...
            reader.onloadend = () => {
                ...
                setHiddenImagePreviewUrl(reader.result);
            }
    ...
    >
        <Box sx={boxStyle}>
            ...
            <Box sx={{ mt: 3, fontWeight: '600', fontSize: '16px' }}>
                ✍️ Message
                <Box sx={{ display: 'inline', verticalAlign: 'super', fontSize: '15px', color: '#FF5B00' }}>
                    *
                </Box>
            </Box>
            <TextField
                ...
                multiline
            />
            <Box sx={{ mt: 3, display: 'flex', fontWeight: '600', fontSize: '16px', alignItems: 'baseline' }}>
                ...
                <Box sx={{ ml: 'auto' }}>
                    ...
                    <Input sx={{ display: 'none' }}
                        ...
                        <Button sx={{ borderRadius: '0', padding: '4px', fontSize: '11px' }} variant="outlined" component="span">
                            Upload
                        </Button>
                    </label>
                </Box>
            ...
            <Box sx={{ mt: 2, gap: 2, display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
                <img style={{ maxHeight: '30vh', maxWidth: '70%' }} src={hiddenImagePreviewUrl} />
            </Box>
            }

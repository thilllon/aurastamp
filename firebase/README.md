...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create traffic-app
  fly apps create traffic-app-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app traffic-app
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app traffic-app-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'traffic-app-staging' does not match app name in config file 'traffic-app'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name traffic-app-db
  fly postgres attach --app traffic-app traffic-app-db

  fly postgres create --name traffic-app-staging-db
  fly postgres attach --app traffic-app-staging traffic-app-staging-db
  ```

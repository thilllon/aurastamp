...
const Results: FC<ResultsProps> = ({ users = [] }) => {
  ...
  const handleTabsChange = (_event: SyntheticEvent, tabsValue: unknown) => {
    ...
    setFilters((prevFilters) => ({ ...prevFilters, role: value }));
  ...
  const handleDeleteCompleted = () => {
    ...
    enqueueSnackbar(t('The user account has been removed'), { variant: 'success' });
  };
                ...
                {paginatedUsers.map((user) => {
                  ...
                  return (
                    <Grid item xs={12} sm={6} md={4} key={user.id}>
                      <CardWrapper className={clsx({ 'Mui-selected': isUserSelected })}>
                        <Box sx={{ position: 'relative', zIndex: '2' }}>
                          ...
                          <Box p={2} display='flex' alignItems='flex-start'>
                            <Avatar sx={{ width: 50, height: 50, mr: 2 }} src={user.image} />
                            <Box>
                              <Box>
                                <Link variant='h5' href='/management/users/single/1'>
                                  {user.name}
                                </Link>

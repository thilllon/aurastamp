...
function Block5() {
  ...
  return (
    <Card>
      <Box display='flex' alignItems='center' justifyContent='space-between' py={2} px={3}>
        <Box>
          <Typography
            variant='caption'
            fontWeight='bold'
          ...
          <List disablePadding>
            {items.map((item) => (
              <Fragment key={item.id}>
                <ListItem sx={{ display: { xs: 'block', md: 'flex' }, py: 2, px: 2.5 }}>
                  <ListItemAvatar sx={{ mr: 2, mb: { xs: 2, md: 0 } }}>
                    <LinkHover href='#'>
                      <Avatar
                        variant='rounded'
                  ...
                  <Box component='span' sx={{ display: { xs: 'none', md: 'inline-block' } }}>
                    <Box ml={3} textAlign='right'>
                      ...
                      <Typography variant='subtitle2'>{item.info_title}</Typography>
                    </Box>
                  </Box>
                </ListItem>
                <Divider />
      ...
      <Box p={3} sx={{ textAlign: 'center' }}>
        <Button variant='contained' endIcon={<ArrowForwardTwoToneIcon />} size='small'>

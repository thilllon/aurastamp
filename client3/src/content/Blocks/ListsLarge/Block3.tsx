...
function Block3() {
  ...
  return (
    <Card>
      <CardHeader
        sx={{ p: 2 }}
        titleTypographyProps={{ component: 'h4', variant: 'h3' }}
        action={
      ...
      />
      <Divider />
      <Box sx={{ background: `${alpha(theme.colors.alpha.black[10], 0.05)}` }}>
        <Box p={3} display='flex' alignItems='flex-start' justifyContent='space-between'>
          <Box>
            ...
            <Typography gutterBottom sx={{ py: 0.5 }} variant='h2'>
              $8,489
            </Typography>
            <Box display='flex' alignItems='center'>
              ...
              <Typography sx={{ pl: 0.2 }} variant='subtitle2'>
        ...
        <Box px={2}>
          <Chart options={Box2Options} series={Box2Data} type='bar' height={148} />
        </Box>
      </Box>
      <Divider />
      <Typography
        fontWeight='bold'
        sx={{ py: 1, px: 2, fontSize: `${theme.typography.pxToRem(13)}` }}
        component='h6'
        variant='caption'
      >
        {t('Top Sellers')}
      ...
      <Box sx={{ height: 263 }}>
        <Scrollbar>
          <List disablePadding>
            {items.map((item) => (
              <ListItem key={item.id} sx={{ py: 1.5, px: 2 }}>
                <ListItemAvatar sx={{ display: 'flex', mr: 1.5 }}>
                  <Box display='inline-flex' position='relative'>
                    ...
                    <CircularProgress
                      ...
                      sx={{
                        color: (theme) =>
                          item.arrow ? theme.colors.error.lighter : theme.colors.success.lighter,
                      }}
                      size={56}
                      thickness={3}
                      value={100}
                    ...
                    <CircularProgress
                      ...
                      sx={{
                        ...
                        left: 0,
                        color: (theme) =>
                          item.arrow ? theme.colors.error.main : theme.colors.success.main,
                        top: 0,

    ...
    <Card variant='outlined'>
      <Box p={3} display='flex' alignItems='center' justifyContent='space-between'>
        <Box>
          ...
          <Typography variant='subtitle2'>{t('These are your performance indicators')}</Typography>
        </Box>
            ...
            {items.map((item) => (
              <Fragment key={item.id}>
                <ListItem sx={{ display: 'block', py: 1.5, px: 3 }}>
                  <Box display='flex' mb={0.6} alignItems='center' justifyContent='space-between'>
                    <Typography variant='h4'>{t(item.title)}</Typography>
                    <Typography sx={{ color: `${theme.colors.primary.main}` }} variant='h3'>
                      {item.value}
                    </Typography>
                  </Box>
                  <LinearProgressPrimary variant='determinate' value={item.progress} />
                  <Box
                    display='flex'
                    sx={{ pt: 0.4 }}
                    alignItems='center'
      ...
      <Divider />
      <CardWrapper elevation={0} sx={{ textAlign: 'center', p: 2 }}>

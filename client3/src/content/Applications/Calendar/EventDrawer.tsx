...
const EventDrawer: FC<AddEditEventModalProps> = ({
  ...
  const handleDelete = async (): Promise<void> => {
    try {
      ...
      onDeleteComplete();
      enqueueSnackbar(t('The event has been deleted'), { variant: 'error' });
    } catch (err) {
      console.error(err);
    }
  };
    ...
    <Formik
      ...
      validationSchema={Yup.object().shape({
        ...
        description: Yup.string().max(5000),
          ...
          (start: Date, schema: any) =>
            start && schema.min(start, t('The end date should be after start date'))
        ),
        start: Yup.date(),
      ...
      onSubmit={async (values, { resetForm, setErrors, setStatus, setSubmitting }) => {
        try {
          ...
          setSubmitting(false);
          enqueueSnackbar(t('The calendar has been successfully updated!'), { variant: 'success' });
        ...
        <form onSubmit={handleSubmit}>
          <Box p={3}>
            <Typography variant='h4'>
              {isCreating ? t('Create new calendar event') : t('Edit calendar event')}
            </Typography>
          </Box>
          <Divider />
          <Box px={3} py={2}>
          ...
          >
            ...
            <Box>
              <Button variant='outlined' sx={{ mr: 1 }} color='secondary' onClick={onCancel}>
                {t('Cancel')}
              </Button>
              <Button
                variant='contained'

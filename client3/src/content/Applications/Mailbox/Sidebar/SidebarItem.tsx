...
import type { Tag } from '@/models/mailbox';
import {
...
export const MailboxSidebarItem: FC<MailboxSidebarItemProps> = (props) => {
  ...
  const href = tag.id !== 'inbox' ? `/apps/mailbox?tag=${tag.id}` : '/apps/mailbox';

  return (
    <ListItemButton sx={{ my: '2px' }} component={Link} href={href} selected={active} {...rest}>
      <ListItemIcon>

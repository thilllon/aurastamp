...
import { ProblemCategoryMap } from '@/types/mappedObject';
...
export const ChallengeCard = ({
  ...
  const hasResults = Boolean(takeCount && takeCount > 0);
  ...
  return (
    <ChallengeCardWrapper sx={{ height: anonymous ? 380 : 430 }} onMouseOver={onMouseOver}>
      <CardContent sx={{ mb: 'auto', display: 'flex', flexFlow: 'column', flex: 1, p: 1.5, pb: 0 }}>
        ...
        >
          {tags.map((tag) => (
            <Chip key={tag} label={ProblemCategoryMap[tag] || tag} size={'small'} />
          ))}
        </Stack>
      ...
      <CardContent sx={{ p: 1.5, flexBasis: 0, gap: 1, display: 'flex' }}>
        <Button
          sx={{ flex: 1 }}
          LinkComponent={Link}
          href={href}
        ...
        {/* {hasResults && (
          <Button LinkComponent={Link} href={`/challenges/${id}/results`}>
            {t('결과보기')}
          </Button>
        )} */}
      </CardContent>

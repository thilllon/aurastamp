...
// create a personal token at https://vercel.com/account/tokens
class VercelClient {
  private readonly vercelClient: AxiosInstance;
  private teamId?: string;
  constructor(auth: string, teamId?: string) {
    this.teamId = teamId;
  ...
  _deployments() {
    return {
      ...
      get: async ({ id, teamId }: { id: string; teamId?: string }) => {
        const url = `/v13/deployments/${id}`;
        const response = await this.vercelClient.get<GetDeploymentByIdResponse>(url, {
          params: { teamId },
        });
      ...
      list: async ({
        ...
        const response = await this.vercelClient.get<ListDeploymentsResponse>(url, {
          params: {
            teamId,
            projectId,
            limit,
      ...
      delete: async ({ id, teamId }: any) => {
        ...
        const url = `/v13/deployments/${id}`;
        const response = await this.vercelClient.get<Response>(url, { params: { teamId } });
        return response.data;
      },
    };
  }
...
const token = 'QsLvLTPictGEEEVwcKzHnTcj';
...
const handler: NextApiHandler = async (req, res) => {
  if (req.method === 'GET') {
    ...
    const result = await vercelClient.deployments.list({
    ...
    const result = await vercelClient.deployments.delete({
...
type GetDeploymentByIdResponse =
  | {
      build: {

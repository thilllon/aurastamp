...
const RenderSidebarMenuItems = memo(
  ({ items, path }: { items: MenuItem[]; path: string }): JSX.Element => (
    <SubMenuWrapper>
      <List component='div'>
        {items.reduce<JSX.Element[]>((ev, item) => reduceChildRoutes({ ev, item, path }), [])}
      </List>
    </SubMenuWrapper>
  )
);
...
const reduceChildRoutes = ({
  ...
  if (item.items) {
    ev.push(
      ...
      >
        <RenderSidebarMenuItems path={path} items={item.items} />
      </SidebarMenuItem>
    );
  } else {
    ev.push(
    ...
    <>
      {menuItems.map((section) => (
        <MenuWrapper key={section.heading}>
          ...
          >
            <RenderSidebarMenuItems path={router.asPath} items={section.items} />
          </List>
        </MenuWrapper>
      ))}
    </>
